 ...
 *
 * This class represents an OMB profile. Do not call the setters with null values.
 * Instead, if you want to delete a field, pass an empty string. The getters will
 * return null!
 *
 * PHP version 5
 *
...
class OMB_Profile {
   ...
   *
   * @param bool $force_all Specifies whether empty fields should be added to the
   *                        array as well. This is neccessary to clear a field via
   *                        updateProfile.
   *
  ...
  public static function fromParameters($parameters, $prefix) {
    ...
    if (isset($parameters[$prefix.'_profile'])) {
      $profile->setProfileURL($parameters[$prefix.'_profile']);
    }

    if (isset($parameters[$prefix.'_license'])) {
      $profile->setLicenseURL($parameters[$prefix.'_license']);
    }

    if (isset($parameters[$prefix.'_nickname'])) {
      $profile->setNickname($parameters[$prefix.'_nickname']);
    }

    if (isset($parameters[$prefix.'_fullname'])) {
      $profile->setFullname($parameters[$prefix.'_fullname']);
    }

    if (isset($parameters[$prefix.'_homepage'])) {
      $profile->setHomepage($parameters[$prefix.'_homepage']);
    }

    if (isset($parameters[$prefix.'_bio'])) {
      $profile->setBio($parameters[$prefix.'_bio']);
    }

    if (isset($parameters[$prefix.'_location'])) {
      $profile->setLocation($parameters[$prefix.'_location']);
    }

    if (isset($parameters[$prefix.'_avatar'])) {
      $profile->setAvatarURL($parameters[$prefix.'_avatar']);
    }
    return $profile;
  }
